...
MCRROOT=/usr/pubsw/packages/mcr/v84
...
while [ 1 ]; do
  ...
  if [ "$proc" == "matlab" ]; then
    ...
    if [ $testrun == "1" ]; then
       ...
       #run_mmps_engine.sh $MCRROOT "$proj" "$cmd"
    ...
    if [ "${clustercmd}" == "" ]; then
       ...
       if [ "${batchname}" == "" ]; then
          ...
          case $clustercmd in
            mmps_engine)
              cmd="isbatchdirjob = 1;${proj}_${batchname}"
              ;;
            mmps_fsurf.sh)
              cmd=${proj}_${batchname}
              ;;
            *)
       ...
       if [ "$testrun" == "1" ]; then
          ...
          echo "now run `echo $cmd|cut -d\; -f2`"
          case $clustercmd in
            mmps_engine)
              run_mmps_engine.sh $MCRROOT "$cmd"
              ;;
          ...
          echo "did run `echo $cmd|cut -d\; -f2`"
       fi
    fi
    if [ "$testrun" == "0" -a "${clustercmd}" != "" ]; then
       if [ ! $clustercmd == "mmps_engine" -a ! $clustercmd == "mmps_fsurf.sh" ]; then
          if [ "$batchname" == "" ]; then
             batchname=$command
          fi
          getJobIDs
